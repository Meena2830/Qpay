...
//   const { transactions, setTransactions, loading, setLoading, error, setError, page, setPage, totalPages } = useAppContext();
...
//           `/api/transaction_history?service_id=111&page=${page}`,  // no https:// here
//           {
//             headers: {
...
//     };
//     fetchTransactions();
//   }, [page, setTransactions, setLoading, setError]);
...
//         <button className="text-primaryGreen">Download Statement</button>
//       </div>
//       <div className="bg-lightGreen p-4 rounded-lg flex justify-between items-center">
//         <p>Today's total collection will be auto-settled by 08:00AM, 24th Aug'25 Tomorrow.</p> {/* Updated date */}
//         <button className="bg-primaryGreen text-white px-4 py-2 rounded">Settle Now!</button>
//       </div>
...
//                   <td className="p-2">{tx.name || 'Lisa'}</td> {/* Adapt to API fields */}
//                   <td className="p-2">{tx.date || '03 Sep, 2021'}</td>
//                   <td className="p-2">From {tx.from || 'QPay'}</td>
//                   <td className="p-2">₹{tx.amount || '1,023'}</td>
//                   <td className="p-2">
//                     <span className={`px-2 py-1 rounded ${tx.status === 'Success' ? 'bg-green-200 text-green-800' : tx.status === 'Failed' ? 'bg-red-200 text-red-800' : 'bg-yellow-200 text-yellow-800'}`}>
//                       {tx.status || 'Pending'}
//                     </span>
//                   </td>
//                   <td className="p-2 text-primaryGreen">View</td>
//                 </tr>
//               ))
//             ) : (
//               <tr>
...
//         <button onClick={() => setPage((prev) => Math.max(0, prev - 1))} disabled={page === 0} className="px-4 py-2 bg-gray-200 rounded">&lt;</button>
//         {Array.from({ length: totalPages }).map((_, i) => (
//           <button key={i} onClick={() => setPage(i)} className={`px-4 py-2 ${page === i ? 'bg-primaryGreen text-white' : 'bg-gray-200'} rounded`}>
//             {i + 1}
//           </button>
//         ))}
//         <button onClick={() => setPage((prev) => Math.min(totalPages - 1, prev + 1))} disabled={page === totalPages - 1} className="px-4 py-2 bg-gray-200 rounded">&gt;</button>
//       </div>
//     </div>
//   );
// };
...
const SettlementHistory = () => {
      ...
      try {
        ...
        try {
          response = await axios.get(
            ...
            {
              headers: {
                Authorization: "Bearer eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoiMjgyMDUiLCJtb2JpbGUiOiI3MzU4MjIxMzU0IiwiYXBwX2lkIjoiNjAiLCJtaWQiOiIzNDgiLCJ0b2tlbiI6IjZjZjFhMzNhZDJkOGQyNjFkMWYwNDBiMWIwZGViMjc1IiwiZ3JvdXBJZCI�IjIxMDYxIiwiaXNzIjoiMjgyMDUifQ.ADopz72M1Z-eKpFXJd04RZvLxXHyJ8fFaT4HnzxxQCk",
              },
            }
          );
        } catch (proxyError) {
          // If proxy fails, try direct API call (for development)
  ...
  // Date formatting function from the first code
 const formatDate = (dateString) => {
  ...
  // Format amount with Yen symbol as shown in UI8 design
...
<button className="ml-4  border border-gray-300 rounded-lg p-2 bg-[#42794A] transition-colors">
  ...
  <span className="text-[#42794A] font-medium">08:00AM, 23rd Oct'22</span> Tomorrow.
</p>

        </div>
        <button onClick={() => setShowModal(true)}className="bg-[#42794A] text-white px-4 py-2 rounded hover:bg-green-600 transition-colors flex items-center space-x-2">
  ...
  IBRAHIM MOHAMMEDALI
        ...
        <div className="flex justify-between mb-1">
          <span className="font-[Gilroy]  text-[14px] leading-[100%] tracking-[0em] text-[#999999]">Past pending amount</span>
          <span className="font-[Gilroy] font-semibold text-[13px] leading-[100%] tracking-[0em] text-[#252525] text-right">092141241127</span>
        </div>
        <div className="flex justify-between">
          <span className="font-[Gilroy]  text-[14px] leading-[100%] tracking-[0em] text-[#999999]">Charges</span>
          <span className="font-[Gilroy] font-semibold text-[13px] leading-[100%] tracking-[0em] text-[#252525] text-right">07, Aug 2024</span>
        </div>
      </div>

      {/* Today's Total Collection repeated */}
  ...
  Settle Now is Chargeable
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        <table className="w-full">
          <thead>
            <tr className="bg-lightGreen text-left">
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">NAME</th>
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">DATE & TIME</th>
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">ACCOUNT</th>
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">AMOUNT</th>
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">STATUS</th>
              <th className="p-4 text-[#999999] font-semibold text-[16px] leading-[16px] tracking-[0.12em] uppercase">DETAILS</th>
            </tr>
          </thead>
          <tbody>
            {transactions.length > 0 ? (
              transactions.map((tx, idx) => (
                <tr key={idx} className="border-b hover:bg-gray-50">
                  <td className="p-4 font-semibold text-[18px] leading-[100%] tracking-[0%] text-[#252525] ">{tx.service_child_name || 'Lisa'}</td>
                  <td className="p-4 font-semibold text-[18px] leading-[100%] tracking-[0%] text-[#252525]">{formatDate(tx.created_date)}</td>
                  <td className="p-4 flex items-center gap-2 text-[#999999] font-medium text-[16px] leading-[100%] tracking-[0%]">
                From{accountMap[tx.aggregator_id] ? (
                  <>
                    <img
                      ...
                      className="w-8 h-8"
                    />
                    {/* {accountMap[tx.aggregator_id].name} */}
                  </>
                ) : (
                  <>
                    <img src="/icons/qpay.png" alt="QPay" className="w-5 h-5" />
                    QPay
                  </>
                )}
              </td>
                  <td className="p-4 font-semibold text-[18px] leading-[100%] tracking-[0%] text-[#252525]">{formatAmount(tx.amount)}</td>
                 <td className="p-4">
  <span
